export function markdownToHtml(markdown: string): string {
  ...
  html = html.replace(/^# (.*?)$/gm, "<h1 class=\"text-4xl font-bold mt-8\">$1</h1>");
  html = html.replace(/^## (.*?)$/gm, "<h2 class=\"text-3xl font-bold mt-6\">$1</h2>");
  html = html.replace(/^### (.*?)$/gm, "<h3 class=\"text-2xl font-bold mt-6\">$1</h3>");
  html = html.replace(/^#### (.*?)$/gm, "<h4 class=\"text-xl font-bold mt-4\">$1</h4>");
  html = html.replace(/^##### (.*?)$/gm, "<h5 class=\"text-lg font-bold mt-4\">$1</h5>");
  html = html.replace(/^###### (.*?)$/gm, "<h6 class=\"text-base font-bold mt-2\">$1</h6>");
  ...
  // Better handling for unordered lists - first convert each list item
  ...
  // Process blockquotes
  html = html.replace(/^> (.*?)$/gm, "<blockquote class=\"pl-4 border-l-4 border-muted-foreground italic my-4 text-muted-foreground\">$1</blockquote>");
  ...
  // Process paragraphs, but avoid wrapping existing HTML elements
  html = html.replace(/(^|\n)(?!<h[1-6]|<ul|<ol|<li|<blockquote|<pre|<p|<\/)[^\n]+(\n|$)/g, "<p class=\"my-4 leading-relaxed\">$&</p>");

  // Clean up any extra paragraph tags around existing elements
  html = html.replace(/<p.*?>(<(?:h[1-6]|ul|ol|li|blockquote|pre).*?<\/(?:h[1-6]|ul|ol|li|blockquote|pre)>)<\/p>/g, "$1");
